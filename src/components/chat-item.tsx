...
const ChatItem: FC<ChatItemProps> = ({
 ...
 const canDeleteMessage =
  !deleted && (isOwner || isSuperAdmin || isRegulator);
 ...
 const onSubmit = async ({ content }: z.infer<typeof formSchema>) => {
  const url = `${socketUrl}/${id}?${new URLSearchParams(
   socketQuery,
  )}`;
  ...
  isEditing ? (
   ...
   <div
    className={cn('text-sm', {
     'text-xs opacity-90 italic': deleted,
    })}
 ...
 return (
  <div className="relative group flex items-center hover:bg-black/5 px-1 py-2 rounded transition w-full">
    ...
    <div className="flex flex-col w-full">
     <div className="flex items-center gap-x-2">
      <Typography
       ...
       className="font-semibold text-sm hover:underline cursor-pointer text-black/60 dark:text-white/60"
      />
      {isSuperAdmin && (
       <MdOutlineAdminPanelSettings className="w-5 h-5 text-black/60 dark:text-white/60" />
      )}
      {isRegulator && <MdOutlineAssistantPhoto className="w-5 h-5 text-black/60 dark:text-white/60" />}
      {isUpdated && !deleted && (
       <span className="text-xs text-black/60 dark:text-white/60">(edited)</span>
      )}
   ...
   {canDeleteMessage && (
    <div className="hidden absolute group-hover:flex flex-row gap-2 border bg-white dark:bg-black dark:text-white text-black rounded-md p-2 top-2 right-0">
     <DeleteDialog />
     {canEditMessage && (
      <Edit
