   ...
   <div className="w-full flex flex-col gap-2 p-3">
    <div>
     ...
     >
      <div className="flex items-center justify-between">
       ...
       <div
        className={cn(
         ...
         color === 'green' && 'hover:bg-green-800',
         color === 'blue' && 'hover:bg-blue-800',
        )}
       >
      ...
      <CollapsibleContent>
       {userWorkspaceChannels &&
        userWorkspaceChannels.map((channel, i) => {
         ...
         return (
          <Typography
           ...
           text={`# ${channel.name}`}
           className={cn(
            'px-2 py-1 rounded-sm !cursor-pointer !text-sm transition-all duration-300',
            `${secondayBg}`,
        ...
        {currentWorkspaceData?.members?.map((member) => {
         return (
          <Typography
           ...
           className={cn(
            'px-2 py-1 rounded-sm cursor-pointer transition-all duration-300',
            `${secondayBg}`,
           )}
